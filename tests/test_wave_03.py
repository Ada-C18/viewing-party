...
def test_friends_unique_movies_not_duplicated():
    # Arrange
    amandas_data = clean_wave_3_data()
    amandas_data_2 = clean_wave_3_data()
    amandas_data["friends"][0]["watched"].append(INTRIGUE_3)

    amandas_data_2["friends"][0]["watched"].append(ACTION_3)

    # Act
    friends_unique_movies = get_friends_unique_watched(amandas_data)
    friends_unique_movies_2 = get_friends_unique_watched(amandas_data_2)
    # Assert
    assert len(friends_unique_movies) == 3
    assert len(friends_unique_movies_2) == 4
