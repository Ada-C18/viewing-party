...
def test_friends_unique_movies_not_duplicated():
    ...
    # **************************************************************************************************
    assert friends_unique_movies.count(friends_unique_movies[0]) == 1
    assert friends_unique_movies.count(friends_unique_movies[1])== 1
    assert friends_unique_movies.count(friends_unique_movies[2]) == 1
    
    for movie in friends_unique_movies:
        if friends_unique_movies.count(movie) == 1:
            assert INTRIGUE_3 in friends_unique_movies
            assert HORROR_1 in friends_unique_movies
            assert FANTASY_4 in friends_unique_movies
