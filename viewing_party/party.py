...
def  get_unique_watched(user_data):
    ...
    for movie in user_data["watched"]:
        if movie not in friends_watched and movie not in unique_list:
...
def get_available_recs(user_data):
    ...
    rec = []
    for movie in friends_watched:
        if movie not in user_data["watched"] and movie in friends_watched and movie["host"] in user_data["subscriptions"]:
            rec.append(movie)
    return rec
...
def get_new_rec_by_genre(user_data):
    get_genres = get_most_watched_genre(user_data)
    friends_watched = get_friends_unique_watched(user_data)
    genre = []
    
    for movie in friends_watched:
        if movie["genre"] == get_genres:
            genre.append(movie)
    return genre
...
def get_rec_from_favorites(user_data):
    ...
    for movie in user_unique:
        if movie in user_data["favorites"]:
            rec_from_favs.append(movie)
            
    print(rec_from_favs)
    return rec_from_favs
